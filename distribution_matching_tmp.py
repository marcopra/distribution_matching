...
class DistributionMatcher:
    ...
    def compute_gradient(self, nu0: np.ndarray, nu_target: np.ndarray) -> np.ndarray:
        """
        Compute gradient of the <gradient_type> divergence w.r.t. policy.
    ...
    def optimize(self, nu0: np.ndarray, nu_target: np.ndarray, 
        ...
        for iteration in range(n_updates):
            ...
            elif self.gradient_type == 'forward':  # forward
                I = np.eye(self.n_states)
                M = self.M_pi_operator(self.policy_operator)
                kl = self.kl_divergence(nu0, (I - self.gamma * M)/(1-self.gamma) @ nu_target)
            elif self.gradient_type == 'MMD':
                nu_pi = self.compute_discounted_occupancy(nu0)
                kl = np.linalg.norm(nu_pi - nu_target)#**2
...
class DistributionVisualizer:
    ...
    def plot_results(self, nu0: np.ndarray, nu_target: np.ndarray, 
        ...
        # save nu final in a txt file save the entire matrix 
        np.savetxt(f"{'/'.join(save_path.split('/')[:-1])}/nu_final.txt", nu_final)
