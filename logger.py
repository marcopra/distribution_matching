...
COMMON_EVAL_DP_FORMAT = [('frame', 'F', 'int'), ('step', 'S', 'int'),
                         ('episode', 'E', 'int'), ('episode_length', 'L', 'int'),
                         ('episode_reward', 'R', 'float'),
                         ('dataset_size', 'DS', 'int'),
                         ('dynamic_programming_cycle', 'DPC', 'int'),
                         ('total_time', 'T', 'time')]
...
class MetersGroup(object):
    ...
    def _prime_meters(self):
        ...
        for key, meter in self._meters.items():
            if key.startswith('eval_dp'):
                key = key[len('eval_dp') + 1:]
    ...
    def dump(self, step, prefix):
        ...
        # if (prefix == 'train' and step % 100 == 0) or prefix != 'train':
...
class Logger(object):
    def __init__(self, log_dir, use_tb, use_wandb):
        ...
        self._eval_mg = MetersGroup(log_dir / 'eval.csv',
                                    ...
                                    use_wandb=use_wandb)
        self._eval_dp_mg = MetersGroup(log_dir / 'eval_dp.csv',
                                       formating=COMMON_EVAL_DP_FORMAT,
                                       use_wandb=use_wandb)
        if use_tb:
    ...
    def log(self, key, value, step):
        assert key.startswith('train') or key.startswith('eval') or key.startswith('eval_dp')
    ...
    def dump(self, step, ty=None):
        ...
        if ty == 'eval_dp':
            self._eval_dp_mg.dump(step, 'eval_dp')
