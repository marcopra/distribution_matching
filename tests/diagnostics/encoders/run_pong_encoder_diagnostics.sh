...
EPISODES_DIR="tests/outputs/atari/salient_episodes_pong"
RESULTS_DIR="tests/outputs/encoders/pong_encoder_results"
NUM_EPISODES=5
RESOLUTION=84
FRAME_STACK=3
ACTION_REPEAT=4
...
if [ "$SKIP_SAMPLING" = false ]; then
    ...
    if [ ! -d "$EPISODES_DIR" ] || [ ! "$(ls -A $EPISODES_DIR)" ]; then
        python "$REPO_ROOT/tests/diagnostics/atari/sample_pong_salient_episodes.py" \
...
python "$REPO_ROOT/tests/diagnostics/atari/inspect_pong_salient_episodes.py" --episodes_dir "$EPISODES_DIR"
...
python "$SCRIPT_DIR/evaluate_pong_encoder_embeddings.py" \
