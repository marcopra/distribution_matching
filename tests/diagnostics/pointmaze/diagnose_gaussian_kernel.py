"""Diagnose PointMaze Gaussian-kernel behavior for saved ROVER agents.

Requires a snapshot and PointMaze config; writes plots and JSON summaries to
tests/outputs/pointmaze/gaussian_kernel by default.
"""
...
def load_snapshot_agent(snapshot_path: Path, device: torch.device):
    ...
    import agent.rover_nystrom_pointmaze_debug  # noqa: F401
...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("tests/outputs/pointmaze/gaussian_kernel"))
