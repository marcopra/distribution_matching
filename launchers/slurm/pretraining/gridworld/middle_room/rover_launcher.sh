        ...
        for sink_idx in "${sink_idxs[@]}"; do
            for discount in "${discounts[@]}"; do
                for feature in "${feature_dims[@]}"; do
                    sbatch --export=SEED="${seed}",ENV="${env}",SINK_IDX="${sink_idx}",DISCOUNT="${discount}",FEATURE_DIM="${feature}" launchers/slurm/pretraining/gridworld/middle_room/rover_base.sh
                done
            done
        done
    done
