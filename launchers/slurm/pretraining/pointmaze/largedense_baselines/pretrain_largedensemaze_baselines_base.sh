...
python pretrain.py \
    ...
    save_snapshot=false \
    snapshots=[] \
    snapshot_dir="models/pointmaze/largedense/states/baselines/${AGENT}/seed_${SEED}" \
    wandb_project=pointmaze_baselines \
    wandb_tag="${AGENT}_largedense_states" \
    wandb_run_name="largedense_states_${AGENT}_seed${SEED}"
