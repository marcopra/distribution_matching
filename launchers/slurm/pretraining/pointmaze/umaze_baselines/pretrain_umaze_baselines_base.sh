...
python pretrain.py \
    ...
    save_snapshot=false \
    snapshots=[] \
    snapshot_dir="models/pointmaze/umaze_goal_1/states/baselines/${AGENT}/seed_${SEED}" \
    wandb_project=pointmaze_baselines \
    wandb_tag="${AGENT}_umaze_states" \
    wandb_run_name="umaze_states_${AGENT}_seed${SEED}"
