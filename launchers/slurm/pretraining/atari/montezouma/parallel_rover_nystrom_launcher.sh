            ...
            for feature_dim in "${feature_dims[@]}"; do
                for kernel_bandwidth_multiplier in "${kernel_bandwidth_multipliers[@]}"; do
                    for linear_projection in "${linear_projections[@]}"; do
                        sbatch \
                            --export=ALL,SEED="$seed",BATCH_SIZE_ACTOR="$batch_size_actor",SUBSAMPLE="$subsample",FEATURE_DIM="$feature_dim",KERNEL_BANDWIDTH_MULTIPLIER="$kernel_bandwidth_multiplier",LINEAR_PROJECTION="$linear_projection" \
