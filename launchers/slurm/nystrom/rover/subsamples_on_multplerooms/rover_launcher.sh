            ...
            for sink_idx in "${sink_idxs[@]}"; do
                for batch_size_actor in "${batch_size_actor_arr[@]}"; do
                    for feature in "${feature_dims[@]}"; do
                        sbatch --export=SEED="${seed}",SUBSAMPLES="${subsamples}",LR_ACTOR="${lr_actor}",SINK_IDX="${sink_idx}",BATCH_SIZE_ACTOR="${batch_size_actor}",FEATURE_DIM="${feature}" launchers/slurm/nystrom/rover/subsamples_on_multplerooms/rover_base.sh
                    done
                done
            done
        done
