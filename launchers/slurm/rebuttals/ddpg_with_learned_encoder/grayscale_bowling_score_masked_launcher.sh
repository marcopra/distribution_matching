...
for seed in $seeds; do
    for i in "${!replay_buffer_dirs[@]}"; do
        sbatch --export=SEED="${seed}",REPLAY_BUFFER_DIR="${replay_buffer_dirs[$i]}",ENCODER_PATH="${encoder_paths[$i]}",FEATURE_DIM="${feature_dims[$i]}" launchers/slurm/rebuttals/ddpg_with_learned_encoder/grayscale_bowling_score_masked_base.sh
    done
done
