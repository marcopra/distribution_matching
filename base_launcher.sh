...
sh launchers/slurm/offline/ddpg_with_learned_encoder/new_bowling_score_masked_launcher.sh
# bash copy_episodes /home/mprattico/distribution_matching/exp_local/2026.03.23/144903_900879_dist_matching/models/pixels/gym/dist_matching/1/snapshot.pt data_offline/new_bowling_score_masked/1M/rover 1000000
# pretrain.py use_wandb=true wandb_project=rover_pong wandb_tag=rover_hparam2 agent.lr_actor=100 agent.pmd_steps=250 eval_every_frames=10_000 num_train_frames=1_000_000 agent.T_init_steps=10000 agent.update_every_steps=5 agent.update_actor_every_steps=5000 env=pong device=cuda seed=1 save_video=false env.render_mode=rgb_array agent.batch_size_actor=5000 agent.curl=false agent.feature_dim=512 agent.hidden_dim=2048 "agent.sink_schedule='linear(0.0, 0.001, 2_000_000)'" replay_buffer_size=1_000_000 agent.pmd_eta_mode=backtracking env=pong_score_masked
