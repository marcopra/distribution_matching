...
class DistributionMatcher:
    ...
    Args:
        ...
        horizon: Horizon factor for occupancy measure
        eta: Learning rate for mirror descent
        gradient_type: Type of KL gradient ('reverse' or 'forward')
    """
    
    def __init__(self, env: TwoRoomsEnv, horizon: int = 100, 
    ...
    def compute_occupancy(self, nu0: np.ndarray, P: np.ndarray = None) -> np.ndarray:
        """
        ...
        occupancy = np.zeros((self.n_states, 1))
        M = self.M_pi_operator(P)
        for t in range(self.horizon):
            prod = np.linalg.matrix_power(M, t) @ nu0
            occupancy += prod
    ...
    def compute_gradient_kl(self, nu0: np.ndarray, nu_target: np.ndarray) -> np.ndarray:
        ...
        if self.gradient_type == 'forward':
            raise NotImplementedError("Forward KL not implemented in no-gamma version")
            r = nu0 / ((I - self.gamma * M) @ nu_target)
            gradient = self.gamma * self.T_operator.T @ r @ nu_target.T
            assert SECOND_TERM == False, "Second term not implemented for forward KL"
        
        elif self.gradient_type == 'reverse':
            first_term = np.zeros((self.n_states*self.n_actions, self.n_states))
            nu_pi = self.compute_occupancy(nu0)
            M = self.M_pi_operator(self.policy_operator)
            occupancy = self.compute_occupancy(nu0)
            one_plus_log_ratio = 1 + np.log(occupancy / nu_target)
            for t in range(self.horizon):
                for k in range(t-1):
                    first_term += (np.linalg.matrix_power(M, k) @ self.T_operator).T @ one_plus_log_ratio @ (np.linalg.matrix_power(M, t-k -1)@nu0).T
            gradient = (1 -self.alpha) * first_term - self.alpha * np.nan_to_num(1 + self.policy_operator/self.uniform_policy_operator, nan=0.0)*int(SECOND_TERM)
        else:
    ...
    def optimize(self, nu0: np.ndarray, nu_target: np.ndarray, 
        ...
        if verbose:
            ...
            print("="*60)
            print(f"Horizon factor H: {self.horizon}")
        ...
        for iteration in range(n_updates):
            ...
            if print_every and (iteration + 1) % print_every == 0:
                ...
                if save_path_prefix:
                    ...
                    nu_current = self.compute_occupancy(nu0)
...
def main():
    ...
    nu_uniform = matcher.compute_occupancy(nu0, matcher.uniform_policy_operator)
    ...
    visualizer.plot_results(
        ...
        save_path='/home/mprattico/dist_tmp/distribution_matching_nogamma_results_uniform.png'
    )
    ...
    matcher.optimize(
        ...
        save_path_prefix='/home/mprattico/dist_tmp/distribution_matching_nogamma_results'
    )
    
    # Compute final distribution
    nu_final = matcher.compute_occupancy(nu0)
    
    # Visualize optimization results
    visualizer.plot_results(
        nu0, nu_target, nu_final,
        save_path='/home/mprattico/dist_tmp/distribution_matching_nogamma_results.png'
    )
    ...
    print(f"Horizon factor H: {matcher.horizon}")
