'''
python pretrain_mepol.py --env FourRooms10  --k 4  --kl_threshold 15 --max_off_iters 30 --learning_rate 0.0001 \
    --num_trajectories 20 --trajectory_length 300 --num_epochs 650 --heatmap_every 10 \
    --heatmap_episodes 100 --heatmap_num_steps 300 --use_backtracking 1 --zero_mean_start 0 \
    --full_entropy_traj_scale 5 --full_entropy_k 4 --num_workers 1 --online
'''
import argparse
...
# from env.mountain_car_wall import MountainCarContinuous
# from env.gridworld_continuous import GridWorldContinuous
# from env.ant import Ant
# from env.hand_reach import HandReach
# from env.humanoid import Humanoid
from env.discretizer import Discretizer, IdentityDiscretizer
from env.wrappers import ErgodicEnv
from agent.mepol_mutti import mepol
from agent.utils import GaussianPolicy, train_supervised, ActorDiscrete
from env.rooms import FourRoomsEnv
import wandb
...
parser.add_argument('--online', action='store_true',
                    help='Wandb online mode')
parser.add_argument('--wandb_project', type=str, default='mepol',
...
exp_spec = {
    'FourRooms10': {
        'env_create': lambda: ErgodicEnv(FourRoomsEnv(
            room_size=10,
            corridor_positions={"horizontal": 2, "vertical": 2},
            start_position=(0,0),
            goal_position=(18,17), #Ignored
            max_steps=args.trajectory_length

        )),
        'discretizer_create': lambda env: IdentityDiscretizer([21, 21], env.unwrapped.idx_to_state),
...
def create_policy(is_behavioral=False):
    ...
    if spec.get('action_type') == 'discrete':
        policy = ActorDiscrete(
            obs_type=spec.get('obs_type'),
            obs_dim=env.unwrapped.n_states,
            action_dim=env.unwrapped.action_space.n,
            feature_dim=50,
            hidden_dim=spec['hidden_sizes'],
            linear=True
        )
    elif spec.get('action_type') == 'continuous' or spec.get('action_type') is None:
...
if args.env == 'FourRooms10':
    env.num_features = env.unwrapped.n_states
    env.action_space = np.array([1,])
...
folder = "results/"
out_path = os.path.join(os.path.dirname(__file__), "results/", args.tb_dir_name,
                        datetime.now().strftime('%Y_%m_%d_%H_%M_%S') +
                        "__" + str(os.getpid()) + "__" +  exp_name
                        )
os.makedirs(out_path, exist_ok=True)

print("Saving log in: ", out_path)
