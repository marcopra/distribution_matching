    ...
    def _create_transition_matrix(self) -> np.ndarray:
        ...
        for s_idx in range(self.n_states):
            ...
            for action in range(n_actions):
                ...
                T[next_idx, col] = 1.0
                if next_cell == (1, 1):  # Example of special handling for terminal state
    ...
    def compute_gradient_kl(self, nu0: np.ndarray, nu_target: np.ndarray) -> np.ndarray:
        ...
        if self.gradient_type == 'forward':
            ...
            nu_pi_over_nu_target = np.clip(
                np.linalg.solve((1-self.gamma)*(I - self.gamma * M), nu0) / nu_target, 
...
def main():
    ...
    n_updates = 1_000_000
    print_every = 40_000  # Save results every <print_every> iterations
    n_rollouts = 1
    horizon = 9
