...
class ReplayBufferStorageParallel:
    ...
    def add(self, time_step, meta, env_id=0):
        ...
        for spec in self._data_specs:
            ...
            current_episode[spec.name].append(value)
        self._record_pending_transitions(env_id)
        if time_step.last():
            episode = dict()
            for spec in self._data_specs:
                value = current_episode[spec.name]
                episode[spec.name] = np.array(value, spec.dtype)
            for spec in self._meta_specs:
                value = current_episode[spec.name]
                episode[spec.name] = np.array(value, spec.dtype)
            episode["env_id"] = np.full(
                next(iter(episode.values())).shape[0],
                env_id,
                dtype=np.int64,
            )
    ...
    def add_batch(self, time_steps, metas):
    ...
    def _record_pending_transitions(self, env_id):
        if not self._transition_views:
            return
        current_episode = self._current_episodes[int(env_id)]
        if 'observation' not in current_episode:
            return

        episode_length = len(current_episode['observation'])
    ...
    def _preload(self):
        ...
        for fn in self._replay_dir.glob('*.npz'):
            eps_len = fn.stem.split('_')[-1]
            self._num_episodes += 1
            self._num_transitions += int(eps_len)

    def _store_episode(self, episode, env_id):
        ...
        unique = time.time_ns()
        eps_fn = f'{ts}_{unique}_{os.getpid()}_{int(env_id)}_{eps_idx}_{eps_len}.npz'
        save_episode(episode, self._replay_dir / eps_fn)


ReplayBufferStorage = ReplayBufferStorageParallel
...
class ReplayBuffer(IterableDataset):
    ...
    def _try_fetch(self):
        ...
        for eps_fn in eps_fns:
            ...
            eps_idx, eps_len = int(parts[-2]), int(parts[-1])
