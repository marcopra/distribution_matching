...
class DistributionMatcher:
    ...
    def compute_nystrom_system_blockwise(
            self,
            phi_all_obs: torch.Tensor,
            phi_sub_obs: torch.Tensor,
        ...
        A = torch.zeros((m, m), device=phi_sub_obs.device, dtype=phi_sub_obs.dtype)
        for start in range(0, int(phi_all_obs.shape[0]), int(block_size)):
            stop = min(start + int(block_size), int(phi_all_obs.shape[0]))
            K_block = self.state_action_kernel(
                phi_all_obs[start:stop],
                phi_sub_obs,
                all_actions[start:stop],
                sub_actions,
            )
            A.add_(K_block.T @ K_block)
            del K_block
        A.add_(self.lambda_reg * K_mm)
        A.diagonal().add_(1e-6)

        effective_components = m if components is None else min(m, int(components))
        with torch.no_grad():
            K_cpu = K_mm.detach().float().cpu()
            K_cpu.diagonal().add_(1e-8)
            _, eig_vecs = torch.linalg.eigh(K_cpu)
            U_r = eig_vecs[:, -effective_components:].to(
                device=phi_sub_obs.device, dtype=phi_sub_obs.dtype
            )
        return A, U_r

    def policy_from_support_blockwise(
            self,
            phi_all_obs: torch.Tensor,
            phi_sub_next: torch.Tensor,
            all_actions: torch.Tensor,
            coeff: torch.Tensor,
            n_actions: int,
            block_size: int,
        ...
        logits = torch.zeros(
            (m, int(n_actions)),
            device=phi_sub_next.device,
            dtype=phi_sub_next.dtype,
        )
        for start in range(0, int(phi_all_obs.shape[0]), int(block_size)):
            stop = min(start + int(block_size), int(phi_all_obs.shape[0]))
            H_block = self.state_kernel(phi_all_obs[start:stop], phi_sub_next)
            weighted_actions = torch.nn.functional.one_hot(
                all_actions[start:stop].to(phi_all_obs.device).long(),
                int(n_actions),
            ).to(dtype=phi_all_obs.dtype)
            weighted_actions.mul_(coeff[start:stop])
            logits.add_(H_block.T @ weighted_actions)
            del H_block, weighted_actions
        logits.add_(coeff[-1])
        return torch.softmax(-logits, dim=1, dtype=logits.dtype)

    def compute_BM_blockwise(
            self,
            A_nystrom: torch.Tensor,
            phi_all_obs: torch.Tensor,
            phi_sub_obs: torch.Tensor,
            phi_sub_next: torch.Tensor,
            all_actions: torch.Tensor,
        ...
        for start in range(0, int(phi_all_obs.shape[0]), int(block_size)):
            stop = min(start + int(block_size), int(phi_all_obs.shape[0]))
            states = phi_all_obs[start:stop]
            actions = all_actions_device[start:stop]
            K_block = self.state_action_kernel(
                states, phi_sub_obs, actions, sub_actions
            )
            H_block = self.state_kernel(states, phi_sub_next)
            M_block = H_block * pi[:, actions].T
            rhs.add_(K_block.T @ M_block)
            del K_block, H_block, M_block
        return torch.linalg.solve(A_nystrom, rhs)

    def apply_B_transpose_blockwise(
            self,
            A_nystrom: torch.Tensor,
            vector: torch.Tensor,
            phi_all_obs: torch.Tensor,
        ...
        result = torch.empty(
            ...
            device=phi_all_obs.device,
            dtype=phi_all_obs.dtype,
        )
        for start in range(0, int(phi_all_obs.shape[0]), int(block_size)):
            stop = min(start + int(block_size), int(phi_all_obs.shape[0]))
            K_block = self.state_action_kernel(
            ...
            result[start:stop] = K_block @ solved
            del K_block
        return result

    def compute_nu_from_BM_compact(
            self,
            BM: torch.Tensor,
        ...
        alpha_tilde = torch.empty((m + 1, 1), device=alpha.device, dtype=alpha.dtype)
        alpha_tilde[:-1] = alpha
        alpha_tilde[-1] = 1.0
        ...
        tilde_phi_T[d - 1, :m] -= sink_norm * phi_sub_obs.sum(dim=1)
        tilde_phi_T[d - 1, m] = sink_norm
        return (tilde_phi_T @ inv_term).mul_(1 - self.gamma)
    ...
    def compute_gradient_coefficient_compact_blockwise(
            self,
            BM: torch.Tensor,
            A_nystrom: torch.Tensor,
            phi_all_obs: torch.Tensor,
        ...
        S = BM * (-self.gamma)
        idx = torch.arange(m, device=S.device)
        S[idx, idx] += 1.0
        S_r = eig_vecs_r.T @ S @ eig_vecs_r
        S_r_reg = S_r.clone()
        idx_r = torch.arange(r, device=S.device)
        S_r_reg[idx_r, idx_r] += eps
        ...
        tilde_phi_T[d - 1, :m] -= sink_norm * phi_sub_obs.sum(dim=1)
        tilde_phi_T[d - 1, m] = sink_norm
        tilde_K_phi = self.state_kernel(tilde_phi_T.T, tilde_phi_T.T)
        K_rr = eig_vecs_r.T @ tilde_K_phi[:-1, :-1] @ eig_vecs_r
        k_re = eig_vecs_r.T @ tilde_K_phi[:-1, -1:]
        k_er_T = tilde_K_phi[-1:, :-1] @ eig_vecs_r
        k_ee = tilde_K_phi[-1:, -1:]
        ...
        alpha_r = eig_vecs_r.T @ alpha
        z_r = torch.linalg.solve(S_r, alpha_r)
        z_e = torch.ones((1, 1), device=S.device, dtype=S.dtype) / (1.0 - self.gamma)
        h_r = K_rr @ z_r + k_re * z_e
        h_e = k_er_T @ z_r + k_ee * z_e
        tmp_main = torch.linalg.solve(S_r_reg.T, h_r)
        tmp_sink = h_e / (1.0 - self.gamma + eps)
        projected = eig_vecs_r @ tmp_main

        gradient = torch.empty(
            (phi_all_obs.shape[0] + 1, 1),
            device=phi_all_obs.device,
            dtype=phi_all_obs.dtype,
        )
        gradient[:-1] = self.apply_B_transpose_blockwise(
            A_nystrom,
            projected,
            phi_all_obs,
            phi_sub_obs,
