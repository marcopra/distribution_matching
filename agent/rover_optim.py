...
class DistributionMatcher:
    ...
    def _solve_regularized_action_blocks(
            self,
            psi: torch.Tensor,
            B: torch.Tensor,
            action_onehot: torch.Tensor
        ) -> torch.Tensor:
        """Solve (ψψᵀ + λI)X = B as independent action blocks.

        For ψ(s, a) = φ(s) ⊗ e_a, rows with different one-hot actions are
        orthogonal. Sorting rows by action gives A = diag(A1, ..., Ak), so each
        block can be solved independently and then scattered back to sample order.
        """
        action_ids = torch.argmax(action_onehot, dim=1)
        sorted_idx = torch.argsort(action_ids)
        sorted_action_ids = action_ids.index_select(0, sorted_idx)
        psi_sorted = psi.index_select(0, sorted_idx)
        B_sorted = B.index_select(0, sorted_idx)
        _, block_sizes = torch.unique_consecutive(sorted_action_ids, return_counts=True)

        block_solutions = []
        start = 0
        for block_size in block_sizes.tolist():
            end = start + block_size
            psi_block = psi_sorted[start:end]
            B_block = B_sorted[start:end]
            A_block = psi_block @ psi_block.T
            A_block = A_block + self.lambda_reg * torch.eye(
                block_size,
                device=A_block.device,
                dtype=A_block.dtype
            )
            block_solutions.append(torch.linalg.solve(A_block, B_block))
            start = end

        solution_sorted = torch.cat(block_solutions, dim=0)
        solution = torch.empty_like(B)
        solution.index_copy_(0, sorted_idx, solution_sorted)
    ...
    def compute_nu_pi(
        ...
        # ** COMPUTATION STEP **
        # Compute B̃M̃ = Ã⁻¹M̃. With one-hot actions, Ã is block diagonal after
        # grouping rows by action, so we solve one smaller linear system per
        # action instead of factoring the full N x N matrix.
        BM = self._solve_regularized_action_blocks(
            psi_all_obs_action,
            M,
    ...
    def compute_BM(
        ...
        """Compute BM = (ψψᵀ + λI)⁻¹M."""
        return self._solve_regularized_action_blocks(psi, M, action_onehot=action_onehot)
    ...
    def compute_gradient_coefficient(
        ...
        # ** COMPUTATION STEP **
        # Compute BM = A⁻¹M using action-wise blocks of A = ψψᵀ + λI.
        BM = self._solve_regularized_action_blocks(
            psi_all_obs_action,
            M,
            action_onehot=action_onehot
        ...
        # Left term: Ã⁻ᵀ(I - γB̃M̃)⁻ᵀΦ̃. The augmented final row/column is the
        # scalar 1, while the top-left A block uses the same action-wise solve.
        left_term = torch.empty_like(symmetric_term)
        left_term[:-1] = self._solve_regularized_action_blocks(
            psi_all_obs_action,
            symmetric_term[:-1],
            action_onehot=action_onehot
        )
...
class RoverAgent:
    ...
    def _cache_features(self, obs, action, next_obs, encoder=None):
        ...
        with torch.no_grad():
            ...
            action = action.long().reshape(-1).to(self.device)
            self.E = F.one_hot(
                action,
                self.n_actions,
            ).to(dtype=self._phi_all_obs.dtype, device=self.device)
