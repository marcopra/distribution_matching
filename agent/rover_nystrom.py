...
class RoverAgent:
    ...
    def _get_actor_update_data(self, replay_iter, obs, action, next_obs, reward, replay_buffer=None):
        if replay_buffer is not None and hasattr(replay_buffer, 'get_all_data'):
            actor_data = self._load_actor_batch_from_replay_buffer(replay_buffer)
        else:
            actor_data = self._load_actor_batch_from_replay_iter(
                replay_iter,
                obs,
                action,
                next_obs,
        ...
        return self._maybe_subsample_actor_batch(*actor_data)
