...
def detect_subplot_structure(csv_path):
    ...
    # Verifica se le sottocartelle contengono CSV
    subplot_data = {}
    for subdir in subdirs:
        subdir_path = os.path.join(csv_path, subdir)
        ...
        has_task_subdirs = any(os.path.isdir(os.path.join(subdir_path, d)) for d in os.listdir(subdir_path))
...
def read_subplot_title(subplot_path):
    """
    Legge il titolo del subplot da un file .txt nella cartella.
    ...
    txt_files = [f for f in os.listdir(subplot_path) if f.endswith('.txt')]
    
    if txt_files:
        # Usa il primo file .txt trovato
        title_file = os.path.join(subplot_path, txt_files[0])
        try:
...
def plot_multiple_subplots(subplot_aggregated_data, subplot_titles, 
    ...
    for idx, subplot_name in enumerate(subplot_names):
        ...
        if idx >= (n_rows - 1) * n_cols:  # Ultima riga
            if x_label:
                ...
                if x_scale_factor is not None and not log_x:
                    x_scale_notation = f"1e{int(np.log10(x_scale_factor))}"
                    label = f"{x_label} ({x_scale_notation})"
                ax.set_xlabel(label)
            else:
        ...
        if idx % n_cols == 0:  # Prima colonna
            if y_label:
                ...
                if y_scale_factor is not None and not log_y:
                    y_scale_notation = f"1e{int(np.log10(y_scale_factor))}"
                    label = f"{y_label} ({y_scale_notation})"
                ax.set_ylabel(label)
            else:
    ...
    # Ordina legenda per algoritmi
    sorted_indices = sorted(range(len(labels)), key=lambda i: labels[i])
    handles = [handles[i] for i in sorted_indices]
    labels = [labels[i] for i in sorted_indices]
    
    # Posiziona legenda in alto a destra della figura
    fig.legend(handles, labels, loc='upper right', bbox_to_anchor=(0.98, 0.98), 
              ncol=1, frameon=True, fontsize=10)
    
    # Titolo generale se fornito
    if overall_title:
        fig.suptitle(overall_title, fontsize=16, y=0.995)
    
    plt.tight_layout(rect=[0, 0, 0.85, 0.99 if overall_title else 1.0])
